...
def lamda_min(d,na):
    ...
    return lamda_min_temp

def M(R,d,na):
    '''Takes the units for the wavelength d in microns and R in meters'''
...
class lens():
    ...
    def __init__(self,f,R,achromat = True,lambd_f = lambd_f,lambd_d = lambd_d,lambd_c = lambd_c):
        ...
        if achromat:
            ...
            self.lamda_min = lamda_min(lambd_d,self.nad)
            self.M = M(self.R,lambd_d,self.nad)
